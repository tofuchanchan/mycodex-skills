...
13. Load `references/figma-qa-checklist.md` and inspect the modified design with Figma screenshot, metadata, visual-semantic inventory results, text overflow audit results, style consistency findings, artifact findings, and layer organization audit results.
...
- Do not recreate, replace, or repair a scoped area while dropping visible logo marks, icons, tags, badges, status chips, count bubbles, or stepper indicators in that area.
- Do not flatten tag/badge semantics into menu or label text. A small rounded `618狂欢` label is a tag; a red `60` beside `VAT信件` is a count badge.
- Do not change segmented steppers into independent pills. Preserve first-only left radii, last-only right radii, middle square segments, dividers, check icons, and active pointers unless the source screenshot proves otherwise.
...
- Do not mark QA as `Pass` while approved or affected areas are missing inventoried logo/icon/tag/badge/stepper details, or while metadata shows flattened strings such as `购买服务 618狂欢`, `续费服务 618狂欢`, or `VAT信件 60` as one text node.
...
- Component boundary: exact UI unit that owns a logo, icon, tag, badge, status chip, segmented stepper, or menu item. Preserve its internal child roles when editing.
...
- Visual-semantic preservation: inventoried logos, icons, promo tags, count badges, status chips, and segmented steppers in the approved or affected area remain present and keep their roles.
- Tag and badge semantics: colored rounded labels stay separate child nodes, not merged into adjacent menu/text labels.
- Segmented steppers: first/last corner radii, middle square segments, dividers, check icons, and active/complete states match the screenshot.
...
- Component grouping: multi-node buttons, fields, selects, file chips, upload controls, tabs, tags, count badges, status badges, segmented steppers, and operation links are grouped as the controls a human would edit together.
